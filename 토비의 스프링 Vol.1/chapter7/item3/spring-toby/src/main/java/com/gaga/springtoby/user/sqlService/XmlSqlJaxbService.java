...
public class XmlSqlJaxbService implements SqlService {
	...
	public XmlSqlJaxbService() {
		/* 생성자에서 예외가 발생할 수 있는 복잡한 초기화 작업을 다루는 것은 좋지 않으므로 별도의 초기화 메소드를 분리한다.
		// JAXB API를 이용해서 XML 문서를 오브젝트 트리롤 읽어온다.
		String contextPath = Sqlmap.class.getPackage().getName();
		try {
			JAXBContext context = JAXBContext.newInstance(contextPath);
			Unmarshaller unmarshaller = context.createUnmarshaller();
			// 리소스 폴더의 usersqlmap.xml 파일을 변환한다.
			InputStream is = UserDao.class.getResourceAsStream("/usersqlmap.xml");
